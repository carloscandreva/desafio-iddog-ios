...
class URLSessionMock: BaseMock, ChallengeURLSession {
...
class BeerServiceTests: BaseTests {
    ...
    func testApiError() {
        //Arrange
        let urlSessionMock = URLSessionMock.init(file: "Beer", error: Errors.generic)
        let expectation = self.expectation(description: "Expect generic error")
    ...
    func testGeneratedURL() {
        ...
        let urlSessionMock = URLSessionMock.init(file: "Beer", error: Errors.offlineMode)
